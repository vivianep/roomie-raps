/*
 * Copyright 2014-2016 CyberVision, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef EVENTFAMILYFACTORY_HPP_
#define EVENTFAMILYFACTORY_HPP_

/*
 *
 * NOTE: THIS FILE IS AUTOGENERATED.
 *
 */


#include <map>
#include <set>
#include <memory>

#include "kaa/KaaDefaults.hpp"
#include "kaa/event/IEventFamily.hpp"
#include "kaa/event/IEventManager.hpp"
#include "kaa/transact/ITransactable.hpp"
#include "kaa/IKaaClientContext.hpp"

#include "kaa/event/gen/LedEventCF.hpp"
#include "kaa/event/gen/MotionEventCF.hpp"
#include "kaa/event/gen/RFIDEventCF.hpp"


namespace kaa {

class IExecutorContext;

class EventFamilyFactory {
public:
    EventFamilyFactory(IEventManager& manager, ITransactable &transactionManager, IKaaClientContext& context)
        : eventManager_(manager), transactionManager_(transactionManager), executorContext_(context.getExecutorContext())
        , context_(context)
    {
                    efcNames_.insert("LedEventCF");
            FQNList LedEventCFEvents =         {"com.roomie.led.LedEvent"} /* {"fqn1","fqn2","fqn3"} */;
            supportedFQNLists_.insert(std::make_pair("LedEventCF", LedEventCFEvents));
            efcNames_.insert("MotionEventCF");
            FQNList MotionEventCFEvents =         {} /* {"fqn1","fqn2","fqn3"} */;
            supportedFQNLists_.insert(std::make_pair("MotionEventCF", MotionEventCFEvents));
            efcNames_.insert("RFIDEventCF");
            FQNList RFIDEventCFEvents =         {} /* {"fqn1","fqn2","fqn3"} */;
            supportedFQNLists_.insert(std::make_pair("RFIDEventCF", RFIDEventCFEvents));

    }

    TransactionIdPtr startEventsBlock() {
        return transactionManager_.beginTransaction(context_);
    }

    void submitEventsBlock(TransactionIdPtr trxId) {
        transactionManager_.commit(trxId, context_);
    }

    void removeEventsBlock(TransactionIdPtr trxId) {
        transactionManager_.rollback(trxId, context_);
    }

        LedEventCF& getLedEventCF()
    {
        if(!LedEventCFInst_){
            LedEventCFInst_.reset(new LedEventCF(eventManager_, context_));
            addEventFamilyByName("LedEventCF", LedEventCFInst_);
        }

        return *LedEventCFInst_;
    }
    MotionEventCF& getMotionEventCF()
    {
        if(!MotionEventCFInst_){
            MotionEventCFInst_.reset(new MotionEventCF(eventManager_, context_));
            addEventFamilyByName("MotionEventCF", MotionEventCFInst_);
        }

        return *MotionEventCFInst_;
    }
    RFIDEventCF& getRFIDEventCF()
    {
        if(!RFIDEventCFInst_){
            RFIDEventCFInst_.reset(new RFIDEventCF(eventManager_, context_));
            addEventFamilyByName("RFIDEventCF", RFIDEventCFInst_);
        }

        return *RFIDEventCFInst_;
    }


private:
    IEventManager& eventManager_;
    ITransactable& transactionManager_;
    std::set<std::string> efcNames_;
    std::map<std::string, std::shared_ptr<IEventFamily> > eventFamilies_;
    std::map<std::string, FQNList > supportedFQNLists_;
    IExecutorContext& executorContext_;
    IKaaClientContext& context_;

    std::shared_ptr<IEventFamily> getEventFamilyByName(const std::string& efcName) {
        auto it = eventFamilies_.find(efcName);
        if (it != eventFamilies_.end()) {
            return it->second;
        }
        return std::shared_ptr<IEventFamily>();
    }

    void addEventFamilyByName(const std::string& efcName, std::shared_ptr<IEventFamily> eventFamily) {
        eventManager_.registerEventFamily(eventFamily.get());
        eventFamilies_[efcName] = eventFamily;
    }

    const FQNList& getSupportedFQNsByFamilyName(const std::string& efcName) {
        auto it = supportedFQNLists_.find(efcName);
        if (it != supportedFQNLists_.end()) {
            return it->second;
        }
        static const FQNList empty;
        return empty;
    }

    const std::set<std::string> &getEventFamilyClassNames() {
        return efcNames_;
    }

        std::shared_ptr<LedEventCF> LedEventCFInst_;
    std::shared_ptr<MotionEventCF> MotionEventCFInst_;
    std::shared_ptr<RFIDEventCF> RFIDEventCFInst_;

};

} /* namespace kaa */


#endif /* EVENTFAMILYFACTORY_HPP_ */
